...
class CTanjiro final : public CPlayer
{
protected:
	explicit CTanjiro(ID3D11Device*& _pDevice, ID3D11DeviceContext*& _pContext);
	explicit CTanjiro(const CTanjiro& _rhs);
...
public:
	...
	// Play End Cut Scene 
	virtual void Play_EndCutScene(const _bool& _bIsAI = false) override;
protected:
	// SetUp AI Controller
...
protected:
	...
	// Control Trail Active
	virtual void			Control_Trail_Active() override;
protected:
	// Skill 1
...
private:
	// 스킬1을 사용할 수 있는지를 체크하는 불값.
	_bool			m_bIsSkill_1 = false;
	// 스킬2을 사용할 수 있는지를 체크하는 불값.
	_bool			m_bIsSkill_2 = false;

public:
	static CTanjiro* Create(ID3D11Device*& _pDevice, ID3D11DeviceContext*& _pContext);
