...
class COneHandSword : public CSword
{
public:
	explicit COneHandSword();
	explicit COneHandSword(CObj * pOwner, const TCHAR * szImgPath, const wstring & strName, const ESWORDID & eSwordID, const int & iPower, const int& iGold);
	virtual ~COneHandSword();
public:
	// CEquip을(를) 통해 상속됨
