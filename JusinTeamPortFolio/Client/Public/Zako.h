...
BEGIN(Client)
class CZako final : public COni
{
protected:
	explicit CZako(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext);
	explicit CZako(const CZako& _rhs);
	virtual ~CZako(void) = default;
public:
...
public:
	static CZako* Create(ID3D11Device*& _pDevice, ID3D11DeviceContext*& _pContext);
	virtual CGameObject* Clone(void* pArg) override;
	virtual void Free(void) override;
public:
	static _bool m_bSkillCheck;
	CUSTIMER m_Timer;
	_float3 m_vMyPos = _float3{};
};
END