...
CTwoHandSword::CTwoHandSword(CObj * pOwner, const TCHAR * szImgPath, const wstring & strName, const ESWORDID & eSwordID, const int & iPower, const int & iGold)
...
void CTwoHandSword::Initialize()
{
	...
	m_v2DrawSize = VECTOR2(240.f, 240.f);
	m_v2Size = VECTOR2(40.f, 80.f);

	m_tSpinTimer.iPassTime = 75;
...
void CTwoHandSword::Late_Update()
{
}

void CTwoHandSword::Render(HDC _hdc, const VECTOR2 & v2ScrollPos)
{
...
void CTwoHandSword::Collision_Actor(CObj *& pObj)
{
	if (false == m_bEnable)
		return;
	if (nullptr == pObj)
...
void CTwoHandSword::Release()
{
}

CItem * CTwoHandSword::Copy_Item()
{
	CTwoHandSword* pTwoHand = new  CTwoHandSword(m_pOwner, m_szImgPath, m_strName, m_eSwordID, m_tStatus.iAtkPower, m_iGold);
	pTwoHand->Set_Name(m_strName);
	return pTwoHand;
}

void CTwoHandSword::Obj_Active(const bool & IsActive)
{
	m_bRotate = IsActive;
	m_bUse = IsActive;
	m_fDistance = 0.f;
	CCharacter* pCharacter = static_cast<CCharacter*>(m_pOwner);
	if (nullptr != pCharacter)
	{
		if(DIR_FOR == pCharacter->Get_Direction())
			m_fRotValue = 90.f;
	}
	m_bCollision = false;
}

void CTwoHandSword::Update_Rect()
{
	VECTOR2 v2Size = m_v2DrawSize / 2.f;
	// Collision Size
	//m_fRotValue += 1.f;
...
void CTwoHandSword::Update_OnwerInfo()
{
	if (true == m_bUse && true == m_bRotate)
	{
		...
		if (nullptr != pPlayer)
		{
			...
			v2Pos.y = -v2Pos.y;
			m_v2CenterPos = v2Pos + v2WeaponPos ;
			m_fRotValue -= 2.f * m_iMultipleValue;
			m_fSpinAngle -= 1.f* m_iMultipleValue;
			m_fDistance += 2.f;
		}
	}
}

void CTwoHandSword::Fall()
{
	if (false == m_bItemPicked)
	{
